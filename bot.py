    ...
    with db_connect() as conn:
        ...
        conn.execute(
            """INSERT INTO token_applications
               ...
               VALUES (?, ?, ?, ?, ?, 'pending', ?)""",
            (tg_id, tg_username, name, reason, contact, datetime.utcnow().isoformat())
        )
        conn.commit()
...
def approve_application(app_id: int, plan: str = "free"):
    with db_connect() as conn:
        ...
        if existing_user:
            conn.execute("UPDATE users SET plan = ? WHERE telegram_id = ?", (plan, tg_id))
        else:
            conn.execute(
        ...
        conn.execute(
            "UPDATE token_applications SET status = 'approved' WHERE id = ?", (app_id,)
        )
        conn.commit()
...
def reject_application(app_id: int):
    with db_connect() as conn:
        ...
        if not app:
            return None
        conn.execute(
            "UPDATE token_applications SET status = 'rejected' WHERE id = ?", (app_id,)
        )
        conn.commit()
    return app[0]
...
def app_action_kb(app_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Одобрить (free)", callback_data=f"approve:{app_id}:free"),
            InlineKeyboardButton(text="⭐ Одобрить (pro)", callback_data=f"approve:{app_id}:pro"),
        ],
...
async def cmd_start(message: Message):
    ...
    if is_registered:
        plan_label = {"free": "🆓 Free", "pro": "⭐ Pro", "business": "🏢 Business"}.get(user[2], user[2])
        text = (
    ...
    await message.answer(text, parse_mode="HTML", reply_markup=main_kb(is_registered))
...
async def admin_show_apps(callback: CallbackQuery):
    ...
    for app in apps:
        ...
        await callback.message.answer(
            ...
            f"🆔 <code>{tg_id}</code>\n🕐 {created_at[:16]}",
            parse_mode="HTML", reply_markup=app_action_kb(app_id)
        )
    await callback.answer()
...
async def admin_approve(callback: CallbackQuery):
    ...
    await callback.message.answer(
        f"✅ Заявка #{app_id} одобрена. Пользователь <b>{result['username']}</b> получил план <b>{plan}</b>.",
        parse_mode="HTML"
    )
    try:
        await bot.send_message(
        ...
        logger.warning(f"Не удалось уведомить {result['tg_id']}: {e}")
...
async def main():
    ...
    if PROXY_URL:
        try:
            ...
            logger.warning(f"Не удалось подключить прокси: {e}, запуск без прокси")
